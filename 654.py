...
class PasswordGenerator:
    def __init__(self):
        self.window = Tk()
        self.window.title("密碼產生器")
        self.window.geometry("450x300")
        # Label Frame
        self.label_frame = LabelFrame(
            self.window, text="請輸入你想產生的密碼位數")
        ...
        generate_btn = Button(
            self.button_frame, text="產生密碼", command=self.generate_random_password)
        generate_btn.grid(row=0, column=0, padx=10)
        # Copy Password Button
        copy_btn = Button(self.button_frame,
                          text="複製密碼", command=self.copy_password)
